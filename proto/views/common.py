...
class SafeFormView(View):
    @method_decorator(csrf_exempt)
    def dispatch(self, request, *args, **kwargs):
        return super(SafeFormView, self).dispatch(request, *args, **kwargs)


class LoginRestrictedView(View):
    @method_decorator(login_required)
    def dispatch(self, request, *args, **kwargs):
        return super(LoginRestrictedView, self).dispatch(request, *args, **kwargs)
...
class MultiFormMixin(ContextMixin):
    ...
    def get_forms(self, form_classes, form_names=None, bind_all=False):
        return dict([
            (key,
             self._create_form(key, klass, (form_names and key in form_names)
                                            or bind_all))
...
class ProcessMultipleFormsView(ProcessFormView):
    ...
    def _process_grouped_forms(self, group_name, form_classes):
        ...
        if all([forms.get(form_name).is_valid() for form_name in
                form_names.values()]):
